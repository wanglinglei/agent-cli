...
6. 如果缺少继续执行必需的信息，例如 git commit message、目标分支、远端名称等，必须调用 ask_user 等待用户输入，不要直接结束并让用户下一次再输入。
7. 处理“提交并推送代码”时，应先检查 git status；默认不要提交 .DS_Store、node_modules、output 或其他明显非代码产物；如果用户未提供 commit message，调用 ask_user 获取。
8. 最后用中文总结成功、失败、取消或拦截原因。
